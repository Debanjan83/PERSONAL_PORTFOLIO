...
export default function MinimalTheme() {
  ...
  return (
    <div className="bg-[#fafaf8] text-[#1a1a1a] min-h-screen" style={{ fontFamily: '"DM Sans", sans-serif' }}>
      {/* Progress */}
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-[#fafaf8]/95 backdrop-blur-sm border-b border-black/5">
        <div className="max-w-6xl mx-auto px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href={PROFILE.social.whatsapp} target="_blank" rel="noreferrer"
              className="text-[#999] hover:text-[#25d366] transition-colors">
              <FaWhatsapp size={17} />
            </a>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-black/5 px-8 py-5 flex flex-col gap-4 bg-[#fafaf8]">
            {NAV.map(s => (
              <button key={s} onClick={() => scrollTo(s)}
      ...
      {/* HERO */}
      <section id="home" className="min-h-screen flex items-center px-8 pt-16">
          ...
          <div className="grid md:grid-cols-2 gap-20 items-center">
            <div className="space-y-8">
              <div className="space-y-3">
                ...
                <h1 style={{ fontFamily: '"Syne", sans-serif' }}
                  className="text-6xl md:text-8xl font-extrabold leading-none tracking-tight text-[#1a1a1a]">
              ...
              <p className="text-[#555] text-xl leading-relaxed max-w-md">{PROFILE.title} — {PROFILE.tagline}</p>
              <div className="flex flex-wrap gap-4">
                <button onClick={() => scrollTo('projects')}
                  className="group px-8 py-3.5 bg-[#1a1a1a] text-white text-sm flex items-center gap-2 hover:bg-[#333] transition-colors">
                  See My Work <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                </button>
                <a href={PROFILE.resumeUrl} download
                  className="px-8 py-3.5 border border-[#ccc] text-[#555] text-sm flex items-center gap-2 hover:border-[#1a1a1a] hover:text-[#1a1a1a] transition-colors">
            ...
            <div className="flex justify-center md:justify-end">
              <div className="relative">
                <div className="w-72 h-72 md:w-80 md:h-80 rounded-2xl overflow-hidden bg-[#e8e6df] relative">
                  <img src={PROFILE.photo} alt={PROFILE.name} className="w-full h-full object-cover"
                    onError={(e) => { e.target.style.display='none'; }} />
                </div>
                <div className="absolute -bottom-4 -right-4 bg-[#1a1a1a] text-white text-xs px-4 py-2 font-medium uppercase tracking-widest">
                  {PROFILE.location}
                </div>
              </div>
            </div>
      ...
      <section id="about" className="py-28 px-8 bg-white">
        <div ref={aboutRef} className="max-w-6xl mx-auto section-reveal">
          ...
          <h2 style={{ fontFamily: '"Syne", sans-serif' }} className="text-4xl font-extrabold mb-12 text-[#1a1a1a]">
            Who I Am
          </h2>
          <div className="grid md:grid-cols-3 gap-12">
            <div className="md:col-span-2 text-[#555] text-lg leading-relaxed space-y-4">
              <p>{PROFILE.bio}</p>
            </div>
      ...
      <section id="contact" className="py-28 px-8 bg-white">
        <div ref={contactRef} className="max-w-4xl mx-auto section-reveal">
          <p className="text-[#ccc] text-xs uppercase tracking-[0.4em] mb-3">Contact</p>
          <h2 style={{ fontFamily: '"Syne", sans-serif' }} className="text-4xl md:text-5xl font-extrabold mb-6 text-[#1a1a1a]">
            Get In Touch
          </h2>
          ...
          <a href={`mailto:${PROFILE.email}`}
            className="inline-flex items-center gap-2 px-8 py-4 bg-[#1a1a1a] text-white text-sm font-medium hover:bg-[#333] transition-colors mb-14">
